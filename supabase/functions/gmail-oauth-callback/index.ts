...
import { mensajeError } from "../_shared/http.ts";
...
export default {
  fetch: async (request: Request): Promise<Response> => {
    ...
    let stateRow: { id: string; user_id: string; user_email: string } | null = null;

    try {
      const stateHash = await sha256(state);
      const now = new Date().toISOString();

      const { data, error: consumeError } = await client
      ...
      if (consumeError) throw new Error(`No se pudo validar la autorización: ${consumeError.message}`);
      if (!data) throw new Error("La autorización venció, ya fue usada o no pertenece a Rafiki.");
      stateRow = data;
      ...
      if (!tokens.refresh_token) {
        throw new Error(
          "Google no devolvió el refresh token. Desconecta Rafiki desde tu cuenta Google y vuelve a autorizar."
        );
      }
      ...
      const detail = mensajeError(error);
      try {
        await client.from("gmail_integration_audit").insert({
        ...
        // El callback sigue redirigiendo aunque la auditoría secundaria falle.
      }
