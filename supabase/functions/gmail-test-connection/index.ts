  ...
  fetch: async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      if (!connection || connection.status !== "connected")
        throw new Error("No hay una cuenta de Gmail conectada.");
      if (!connection.refresh_token_ciphertext || !connection.refresh_token_iv) {
        throw new Error("La conexión no tiene credenciales reutilizables. Debes reconectar Gmail.");
      }
      ...
      try {
        ...
        // No ocultamos el error original si la auditoría secundaria falla.
      }
