...
async function parseGoogleResponse(response: Response): Promise<Record<string, unknown>> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Google OAuth respondió: ${description}`);
  }
  return data;
}
...
export async function exchangeAuthorizationCode(code: string): Promise<GoogleTokenResponse> {
  ...
  return (await parseGoogleResponse(response)) as GoogleTokenResponse;
}
...
export async function getGmailProfile(accessToken: string): Promise<GmailProfile> {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(`No se pudo consultar Gmail: ${String(data.error?.message || response.statusText)}`);
  }
...
export async function revokeGoogleToken(token: string): Promise<void> {
  ...
  if (!response.ok && response.status !== 400) {
    throw new Error(`Google no pudo revocar el acceso (${response.status}).`);
  }
}
