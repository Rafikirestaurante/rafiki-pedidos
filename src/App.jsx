...
export default function App() {
  ...
  async function registrarPedido() {
    ...
    try {
      if (!window.navigator.onLine) {
        ...
        mostrarMensaje(
          "Sin internet: tu pedido quedó guardado en este celular y se enviará automáticamente cuando vuelva la conexión.",
          "warning"
        );
        return;
      }
      ...
      if (error) {
        if (esErrorDeConexion(error)) {
          ...
          setVista("confirmacion");
          mostrarMensaje(
            "Problema de conexión: tu pedido quedó guardado en este celular y se enviará automáticamente cuando vuelva el internet.",
            "warning"
          );
          return;
        }
  ...
  async function registrarPedidoMesa({ items, acompanantes, modoLlevar = false, mesa, cliente, telefono, ubicacion, mesero, tipoPago, observaciones: obsMesa }) {
    ...
    try {
      if (!window.navigator.onLine) {
        ...
        mostrarMensaje(
          `Sin internet: el pedido de ${mesaLimpia} quedó guardado pendiente por enviar. Se reenviará cuando vuelva la conexión.`,
          "warning"
        );
      ...
      if (error) {
        if (esErrorDeConexion(error)) {
          const pendiente = guardarPedidoPendienteOffline(nuevoPedido, { origen: "mesas" });
          mostrarMensaje(
            `Problema de conexión: el pedido de ${mesaLimpia} quedó guardado pendiente por enviar.`,
            "warning"
          );
          return { ...nuevoPedido, id_temporal: pendiente.id_temporal, pendiente_offline: true };
        }
  ...
  }, [guardandoEstadoPedidoId, pedidos, mostrarMensaje, puedeCambiarEstado, registrarAuditoria, confirmarRafiki]);
  ...
  }, [finalizandoPendientes, guardandoEstadoPedidoId, pedidosPendientes, mostrarMensaje, puedeFinalizarPendientes, registrarAuditoria, confirmarRafiki]);
  ...
  }, [eliminandoPedidoId, pedidos, mostrarMensaje, puedeEliminarPedido, registrarAuditoria, confirmarRafiki]);
